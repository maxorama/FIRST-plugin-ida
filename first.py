...
class FIRST_FormClass(idaapi.PluginForm):
    ...
    def view_applied(self):
        ...
        for match in sorted(data.values(), key=cmp_func):
...
class FIRST(object):
    ...
    class Metadata():
        ...
        def get_non_jmp_wrapped_functions():
            ...
            for function_ea in idautils.Functions():
                 ...
                 if func:
                      mnem = idc.print_insn_mnem(func.start_ea)
                      op_type = idc.get_operand_type(func.start_ea,0)
                      if not (('jmp' == mnem) and (op_type == idc.o_mem)):
                          addresses.append(func.start_ea)
        ...
        def populate_function_list():
            ...
            for address in FIRST.Metadata.get_non_jmp_wrapped_functions():
                function_name = idc.get_func_name(address)
    ...
    class Info():
        ...
        def get_file_details():
            ...
            try:
                input_md5 = binascii.hexlify(idc.retrieve_input_file_md5()).decode()
            except:
        ...
        def is_32bit():
            ...
            if (idaapi.IDA_SDK_VERSION < 770):
                ...
                import ida_ida
                return ida_ida.inf_is_32bit_or_higher()
        ...
        def signature(address):
            ...
            for block in fc:
                data = {'start' : block.start_ea, 'end' : block.endEA}
                data['size'] = block.endEA - block.start_ea
                data['bytes'] = IDAW.GetManyBytes(block.start_ea, data['size'])
    ...
    class DB():
        ...
        def save(functions):
            ...
            for function in functions:
                ...
                if (len(max_str) % 2) != 0:
                    max_str = '0' + max_str
                max_str = codecs.decode(max_str,'hex')
                ...
                record_str = bytes.fromhex(record_str).decode('utf-8')

                start = FIRST.DB.record_size - len(max_str)
                idc.set_array_string(tag, key, record_str + data[:start])
                ...
                for i in range(1, records):
                    ...
                    idc.set_array_string(tag, key + i, data[begin:end])
        ...
        def get_function(address=None, function=None):
            ...
            if (len(max_str) % 2) != 0:
                max_str = '0' + max_str
            max_str = bytes.fromhex(max_str).decode('utf-8');
    ...
    class Hook():
        ...
        class IDP(idaapi.IDP_Hooks):
            ...
            _ver = idaapi.get_kernel_version().split('.')[0];
        ...
        class UI(idaapi.UI_Hooks):
            ...
            def widget_visible(self, form, hwnd):
...
class FIRSTUI(object):
    ...
    class UploadAll(Generic):
        ...
        def init_data_layout(self):
            ...
            segment_str = '0x{0.start_ea:08x}-0x{0.endEA:08x}: Name "{1}"'
            if not FIRST.Info.is_32bit():
            ...
            try:
                first_segment = True
                for segment in segments:
                    segment_name = IDAW.SegName(segment.start_ea)
...
FIRST_ICON = codecs.decode(get_first_icon(True),'hex')
